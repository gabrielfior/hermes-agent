...
def _discover(
    ...
    return json.dumps({
        ...
        "caveat": (
            "Results may come from OTHER conversation threads/topics (the session DB "
            "spans all of them). Treat matches as reference material from a different "
            "conversation — do not silently import them as context of the current "
            "thread; confirm with the user before acting on them."
        ),
    }, ensure_ascii=False)
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "current world state.\n\n"
        "THREAD ISOLATION\n\n"
        "  The session DB spans ALL conversation threads/topics and platforms. A match "
        "may belong to a different conversation than the one you are in. Never blend "
        "another thread's plans, facts, or task state into the current thread as if it "
        "were local context — present it as 'from a past/other session' and confirm "
